<template>
    <div class="layout-px-spacing">
                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><span>Daftar Barang</span></li>
                            </ol>
                        </nav>
                    </div>
                </li>
        ...
        <div class="col-xl-12 col-md-3 col-sm-6">
            <b-button tag="a" variant="secondary" class="btn-print action-print" @click="print()">Print</b-button>
        </div>
        <div class="col-xl-12 col-md-3 col-sm-6">
            <b-button tag="a" variant="secondary" class="btn-print action-print" @click="bind_data()">Get</b-button>
        </div>
            ...
            <p class="modal-text">
                <b-form @submit.prevent="submit">
                    ...
                    <b-form-row class="mb-4">
                        <b-form-group label="Nama Barang" class="col-md-6">
                            <b-input type="text" v-model="editItem.nmBarang" placeholder="Nama Barang"></b-input>
                        </b-form-group>
                        <b-form-group label="Stok" class="col-md-6">
                            <b-input type="text" v-model="editItem.stkBarang" placeholder="Password"></b-input>
                        </b-form-group>
                    </b-form-row>
                    <b-form-row class="mb-4">
                        <b-form-group label="Hpp" class="col-md-6">
                            <b-input type="text" v-model="editItem.hrgPokok" placeholder="Hpp"></b-input>
                        </b-form-group>
                        <b-form-group label="Hrg Jual" class="col-md-6">
                            <b-input type="text" v-model="editItem.hrgJual" placeholder="Harga Jual"></b-input>
                        </b-form-group>
                    </b-form-row>
...
<script>
    ...
    export default {
        ...
        computed: {
            // ...mapActions(["GetPosts"]),
            ...mapGetters(['StateBarang']),
        },
        methods: {
            ...mapActions(['GetBarang', 'EditBarang']),
            async submit() {
                const Brg = new FormData();
                Brg.append("id", this.editItem.id);
                Brg.append("nmBarang", this.editItem.nmBarang);
                Brg.append("hrgPokok", this.editItem.hrgPokok);
                Brg.append("hrgJual", this.editItem.hrgJual);
                Brg.append("merek", this.editItem.merek);
                try {
                    await this.EditBarang(Brg);
            ...
            bind_data() {
                this.columns = [
                    { key: 'kdBarang', label: 'Kode' },
                    { key: 'nmBarang', label: 'Nama' },
                    { key: 'hrgPokok', label: 'Hrg Pokok' },
                    { key: 'hrgJual', label: 'Hrg Jual' },
                    { key: 'satuanBarang', label: 'Satuan' },
                    { key: 'merek', label: 'Merek' },
                    { key: 'stkBarang', label: 'Stok' },
                    { key: 'action', label: 'Actions', class: 'actions text-center' }
                ];

                this.items = this.StateBarang.data;
                // this.items = [
